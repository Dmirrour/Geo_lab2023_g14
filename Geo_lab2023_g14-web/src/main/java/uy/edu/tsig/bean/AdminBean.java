...
public class AdminBean implements Serializable {
    ...
    public void addAmbulancia() throws IOException {
        double grados = 9.41090001733132E-4;// Probado hasta que encontrara saque una equivalencia en grados este numero
                                            // equivale a esto+-5m
        ...
        try {
            ...
            try {
                ...
                if (resultSet.next()) {
                    ...
                    eC.redirect(eC.getRequestContextPath() + "/admin/indexAdm.xhtml?faces-redirect=true"); // Reemplaza
                                                                                                           // con la URL
                                                                                                           // de la
                                                                                                           // página de
                                                                                                           // confirmación
                    ...
                    addMensaje("Ambulancias", msj, "advertencia");// creo que ninguno de los mensajes se captan en menu
                                                                  // creo que es porque se recarga la paguina y no se ve
            ...
            addMensaje("Ambulancias", msj, "error");// creo que ninguno de los mensajes se captan en menu creo que es
                                                    // porque se recarga la paguina y no se ve
        }
    }
    ...
    public void modAmbu() {
        ...
        try {
            conn = DriverManager.getConnection(url, usuario, contraseña);
            Statement stmt = conn.createStatement();
            double grados = 9.41090001733132E-4;// Probado hasta que encontrara saque una equivalencia en grados este
                                                // numero equivale a esto+-5m
            double des;
            if (codigo.isEmpty()) {
            ...
            } else {// me ahorro la query sql ya que es mallor el buffer no va a quedarse sin buffer
                    // aunque no se cambien el recorrido
                modAaux(amb, "ahiSE", novacio);
            }
            ...
            if (!rec.isEmpty()) {// si no es vasio entro al modifico
                ...
                if (resultSet.next()) {
                    ...
                    if (per) {// solo se ejecuta en el caso de que se alla modificado el buffe y no quedara
                              // cubierto por el anterior recorrido
                        modAaux(amb, "ahiSE", novacio);
                    }
     ...
     * FacesMessage mensaje = new FacesMessage(FacesMessage.SEVERITY_INFO, summary,
     * detail);
